...
**Every page except the landing page carries one `BreadcrumbList` JSON-LD block, and it is the only structured data on the site.** There is no visible breadcrumb and none was added. Trails are built in `src/data/breadcrumbs.ts` by walking the URL upwards and keeping **every ancestor that is a real page** — hubs (`HUB_NAMES`, named with the site's own nav labels) and rooms/experiences (named from the same resolvers the routes use), so `/ubud/villa/honeymoon/pool` passes through the honeymoon room and `/ubud/wellness/yoga/retreat` through the Yoga class. An earlier hubs-only version skipped those and was flagged in review; don't reintroduce it. `PARENT` only *adds* a parent where the URL has none worth naming (root-level property pages, `/ubud/fitness`, the misspelt `/ubud/discoverl`), never replaces one the URL passes through. `components/seo/BreadcrumbJsonLd` renders it, from exactly one place per page: `ManagedPage` (all hand-written routes), `PostPage` (all posts), and the room, experience, legal and `[...slug]` routes themselves. **A new hub page needs an entry in `HUB_NAMES`**, or it will be skipped as an ancestor and its own breadcrumb will fall back to nothing.
...
**The sixth set is the media library, and it is the first one about images rather than pages.** Every photograph and menu PDF this build serves sits in `public/uploads/` under WordPress's own `/YYYY/MM/name.ext` layout, which is what makes the general rule a single line — `/wp-content/uploads/:path*` → `/uploads/:path*`. Measured against every upload URL the Internet Archive holds for the domain (543), **85 resolve exactly**; the rest are files this build does not carry and 404 either way, so the rule can only help. No page here links to `/wp-content` (0 across the prerendered build), so it adds a hop to nothing. Swept against a production build afterwards, **90 of the 543 answer 200** — those 85 plus the five archived members of the named families below, which is the arithmetic checking out rather than a second measurement.

**Six named rules sit in front of it, and the reason they exist is the reason to distrust a list of image URLs.** The client sent five to redirect “for seo purposes”, and none of the five is a file this build carries. Worse, **a redirect per exact URL would have missed most of the links**: WordPress publishes several size variants per image, the client's list was the `-212x300` thumbnails, and Ahrefs reports the referring domains on `-724x1024` and on the bare filename — while the strongest member of the family, `2018/07/In-villa-candle-light-dinner.jpg` at six referring domains, was not on the list at all. So each rule matches a **base name with an optional `-WxH` suffix**, not one spelling, and the candle-light dinner family spans four folders and both of the live site's spellings (`dinner` and the misspelt `diner`).

**The destination is always an image, never a page.** An image URL sent to an HTML page is a soft 404 to Google Images and throws away exactly the signal the redirect was for. `honeymoon-suite-pool-villa.jpeg` goes to `Honeymoon-Suite-Pool-Villa-1.webp`, which is the hero of the Seminyak room titled “Honeymoon Suite Pool Villa”; the whole candle-light dinner family goes to `Couple-having-a-romantic-dinner-at-Nyuh-Bali.jpg`, the one photograph of that offer this build has. One destination for five names is right here — they are one subject, and picking different pictures to keep the URLs distinct would put a photograph behind a name that does not describe it.

**`/wp-content/uploads/2016/02/easter.jpg` is deliberately not redirected**, and it is the fifth URL on the client's own list. It was a 2016 seasonal promotion: there is no Easter photograph here, nothing whose name matches, and the nearest candidate (`sweet-celebration.webp`) is a romantic bed setup with heart balloons — a different occasion wearing the same word. The Internet Archive never captured the file so it cannot even be looked at, and Ahrefs reports no referring domain, so the 404 costs nothing. Inventing a match is how a redirect starts lying.

**These are the redirects whose matching cannot be read off the source**, which is the `src/proxy.ts` lesson applied: every pattern was checked against a running production build, **13 positive cases landing on the right image and 3 negative ones falling through to the wildcard** rather than over-matching. **The seven image URLs carrying the most links have the same treatment now**, ranked by referring domains: 27, 20, 14, 7, 7, 6 and 5. None was a file this build carries, so each is a judgement, and each is commented at its entry. **Only two of the seven survive in the Internet Archive** — the rest were matched on the name alone, which is what the client's rule allows and also what makes the next paragraph the important one.

**Looking at the two that could be looked at is what stopped one of them going to the wrong resort.** `2018/07/contact.jpg` reads as the Seminyak contact photograph and is not: the archived file is **Ubud's** entrance sign, carrying Ubud's street address and `info@ubudnyuhbali.com`. Nothing in the filename says so. It is the same trap as `Seminyak-slider-4.webp` being a housekeeper cleaning a basin, and the same rule answers it — **look at every candidate, and at every original that still exists, before using it.** The other archived one, `ubud-nyuh-bali-about-us.jpg`, turned out to be the entrance bridge and its guardian statues, so it goes to `/ubud`'s own hero: `/ubud` *is* the About Us page that URL names.

Two more of the seven are worth knowing. The 27-referring-domain one was never a photograph of the resort at all — by its filename, `Balance-Meditation-Zen-Stack-Stones-Rocks-Pile-2907290.jpg`, it was stock, agency id and all — so there is nothing here to match it with, and `restoring-body-balance-1.webp` was chosen on the word the old name leads with. And the spa one is a photograph of a printed **menu**, which this build no longer has in any form (the menus are PDFs), so it goes to the lead photograph of the page that carries them. **Where a numbered family is one subject the rule takes the whole family** (`nyuhbalispamenu\d+`, `nyuh-bali-tour-\d+`, the honeymoon suite); where only one index is linked it is pinned to that index (`…one-bedroom-pool-villas-3` → `One-Bedroom-Pool-Villa-3.webp`), because inventing the siblings would put a photograph behind a number that never existed.
